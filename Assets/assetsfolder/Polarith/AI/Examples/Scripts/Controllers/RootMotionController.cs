        ...
        private void Update()
        {
            ...
            if (ObjectiveAsSpeed >= 0 && ObjectiveAsSpeed < Context.DecidedValues.Count)
            {
                ...
                if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out Shot))
                {
                    ...
                    if (TargetDistance >= AllowedDistance)
                    {
                        ...
                        TheNPC.GetComponent<Animator>().SetFloat("Speed", magnitude * speedMultiplier);
                    ...
                    {
                        FollowSpeed = 0;
                        TheNPC.GetComponent<Animator>().Play("attack", -1, 0f);
                    }
                }
            ...
            else
            {
                ...
                if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out Shot))
                {
                    ...
                    {
                        FollowSpeed = 0.01f;
                        TheNPC.GetComponent<Animator>().SetFloat("Speed", 0.1f * speedMultiplier);
                        transform.position = Vector3.MoveTowards(transform.position, ThePlayer.transform.position, FollowSpeed);
